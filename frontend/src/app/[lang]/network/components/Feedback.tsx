...
const NEXT_DEMO_MODE = process.env.NEXT_PUBLIC_DEMO_MODE === "true";
...
const positiveContent = {
  id: 3,
  show: true,
  header: "Gamings positiva påverkan på problemen",
  body: "Här nedan kan du se vilka hjälpande faktorer du angav kopplat till gaming, och vilka problem de hjälper mot.",
  zeroLinks: "Du har inte angivet några hjälpande aspekter av gaming",
};
...
const Feedback = ({ data, dispatch, content }: { data: FeedbackData, dispatch: React.Dispatch<any>, content: FeedbackContent }) => {
  ...
  // Centrality
  // TODO filter link size > 50, or use d3.scalePow to weight values

  // TODO: Memoize
  const problemNodes = data.nodes.filter((node) => node.type !== "positive");
  const problemLinks = data.links.filter(
    (link) => link.source.type !== "positive" && link.target.type !== "positive"
  );

  const positiveLinks = data.links.filter(
    (link) =>
      link.source.type === "positive" ||
      (link.source.nodeId === "gaming" && link.target.type === "positive")
  );
  const positiveNodesIds = positiveLinks.map((d) => d.target.id);
  const positiveNodes = data.nodes.filter(
    (d) => [...positiveNodesIds].includes(d.id) || d.nodeId === "gaming"
  );
      ...
      problemNodes
        ...
        .map((node: Node) => {
          const sum = data.links
            .filter((link: LinkData) => link.source.id == node.id && link.target.type !== "positive")
      ...
      <div className="p-4">
        ...
        {targetNodeOutgoing?.show && (
          <TargetNodeOutgoingFeedback
            data={{ nodes: problemNodes, links: problemLinks }}
        ...
        <Divider className="my-4" />
        <TargetNodeOutgoingFeedback
          data={{ nodes: positiveNodes, links: positiveLinks }}
          content={positiveContent}
          targetNode={positiveNodes.filter((d) => d.nodeId === "gaming")[0]}
          targetId={positiveNodes.filter((d) => d.nodeId === "gaming")[0].id}
          dispatch={dispatch}
        />
        <Divider className="my-4" />
        <div>
