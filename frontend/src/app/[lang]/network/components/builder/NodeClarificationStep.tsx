...
const NodeClarificationStep = ({ nodes, displayRequirementError, content, dispatch, onboarding }) => {
    ...
    return (
        <Card>
            <CardHeader className="flex flex-col gap-0 sm:gap-3 py-1 sm:py-3 bg-white">
                <h2 className="text-md sm:text-xl font-bold">{title}</h2>
                {introduction &&
                    <Markdown
            ...
            <Divider />
            <ScrollShadow hideScrollBar={false} size="60" className="[&>*:last-child]:pb-6 sm:[&>*:last-child]:pb-2">
                {
                    nodesFiltered
                        .map(
                            (node, index) =>
